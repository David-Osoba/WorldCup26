...
def main():
    print("=" * 80)
    print("                La Liga Odds API Execution Script                  ")
    print("=" * 80)
    ...
    print("\n[STEP 1] Scraping La Liga odds from The Odds API...")
    scraped_odds = scraper.scrape_odds("soccer_spain_la_liga")
    if not scraped_odds:
    ...
    # 4. Define La Liga Fixtures (Dynamically parsed from scraped odds)
    fixtures = []
    la_liga_managers = {
        "Real Madrid": "Carlo Ancelotti",
        "Barcelona": "Hansi Flick",
        "Atletico Madrid": "Diego Simeone",
        "Sevilla": "Francisco Javier García Pimienta",
        ...
        "Getafe": "Unknown",
        "Las Palmas": "Unknown",
        "Rayo Vallecano": "Unknown",
        "Mallorca": "Unknown",
        "Leganés": "Unknown",
        "Leganes": "Unknown",
        "Espanyol": "Unknown",
        "Real Valladolid": "Unknown"
    }
    ...
    for key in scraped_odds.keys():
        ...
        # Resolve to canonical names to make manager lookup robust
        home_team = resolver.resolve_team(raw_home)
        away_team = resolver.resolve_team(raw_away)
        
        home_mgr_name = la_liga_managers.get(home_team, "Unknown")
        away_mgr_name = la_liga_managers.get(away_team, "Unknown")
    ...
    for fixture in fixtures:
        ...
        # Generate Match ID following strict convention: laliga_20260524_<home_pref>_<away_pref>
        home_pref = "".join([c for c in home_team if c.isalnum()]).lower()[:3]
        away_pref = "".join([c for c in away_team if c.isalnum()]).lower()[:3]
        match_id = f"laliga_20260524_{home_pref}_{away_pref}"
        ...
        # ----------------------------------------------------
        # Log to SQLite Database (1X2 Prediction)
        # ----------------------------------------------------
        ...
        pred_data = {
            "id": match_id,
            "match_date": "2026-05-24 18:00:00",
            "home_team": home_team,
            "away_team": away_team,
    ...
    for file_path in ["predictions/laliga_accumulator_ticket.json", "laliga_accumulator_ticket.json"]:
        with open(file_path, "w", encoding="utf-8") as f:
            json.dump(premium_parlay, f, indent=2)
    print(f"[INFO] Premium parlay ticket written to 'predictions/laliga_accumulator_ticket.json' and 'laliga_accumulator_ticket.json'")
    
    # ----------------------------------------------------
    # 6. Print EV Summary Table
    # ----------------------------------------------------
    print("\n" + "=" * 110)
    print("                         LA LIGA EXPECTED VALUE (EV) ALL VALUE BETS")
    print("=" * 110)
